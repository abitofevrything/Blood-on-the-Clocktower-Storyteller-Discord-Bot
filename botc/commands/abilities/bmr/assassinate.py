...
from botc import check_if_is_player, check_if_is_night, check_if_dm, RoleCannotUseCommand, \
    check_if_player_really_alive, check_if_can_assassinate, PlayerParser, AbilityForbidden, \
...
class Assassinate(commands.Cog, name = documentation["misc"]["abilities_cog"]):
    """BoTC in-game commands cog
    Assassinate command - used by assassin
    """
    ...
    @commands.command(
        ...
        name = "assassinate",
        hidden = False,
        brief = documentation["doc"]["assassinate"]["brief"],
        help = documentation["doc"]["assassinate"]["brief"],
        description = documentation["doc"]["assassinate"]["description"]
    )
    ...
    @commands.check(check_if_can_assassinate)  # Correct character -> RoleCannotUseCommand
    async def assassinate(self, ctx, *, killed: PlayerParser()):
        """Assassinate command
        usage: assassinate <player>
        characters: assassin
        """
        player = BOTCUtils.get_player_from_id(ctx.author.id)
        await player.role.ego_self.register_assassinate(player, killed)
