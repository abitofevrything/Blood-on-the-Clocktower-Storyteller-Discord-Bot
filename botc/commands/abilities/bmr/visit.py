...
from botc import check_if_is_player, check_if_is_night, check_if_dm, RoleCannotUseCommand, \
    check_if_player_really_alive, check_if_can_visit, PlayerParser, AbilityForbidden, \
...
class Visit(commands.Cog, name = documentation["misc"]["abilities_cog"]):
    """BoTC in-game commands cog
    Visit command - used by sailor
    """
   ...
   # ---------- VISIT COMMAND (Sailor) ----------------------------------------
    @commands.command(
        pass_context = True,
        name = "visit",
        hidden = False,
        brief = documentation["doc"]["visit"]["brief"],
        help = documentation["doc"]["visit"]["help"],
        description = documentation["doc"]["visit"]["description"]
    )
    ...
    @commands.check(check_if_can_visit)  # Correct character -> RoleCannotUseCommand
    async def visit(self, ctx, *, visited: PlayerParser()):
        """Visit command
        usage: visit <player> and <player> and...
        characters: sailor
        """
        player = BOTCUtils.get_player_from_id(ctx.author.id)
        await player.role.ego_self.register_visit(player, visited)
