...
from botc import check_if_is_player, check_if_is_night, check_if_dm, RoleCannotUseCommand, \
    check_if_player_really_alive, check_if_can_execute, PlayerParser, AbilityForbidden, \
...
class Execute(commands.Cog, name = documentation["misc"]["abilities_cog"]):
    """BoTC in-game commands cog
    Excorcise command - used by exorcist
    """
    ...
    # ---------- EXECUTE COMMAND (Godfather) ----------------------------------------
    @commands.command(
        pass_context = True, 
        name = "execute",
        hidden = False,
        brief = documentation["doc"]["execute"]["brief"],
        help = documentation["doc"]["execute"]["help"],
        description = documentation["doc"]["execute"]["description"]
    )
    ...
    @commands.check(check_if_can_execute)  # Correct character -> RoleCannotUseCommand
    async def execute(self, ctx, *, executed: PlayerParser()):
        """Execute command
        usage: execute <player> and <player> and...
        characters: godfather
        """

        player = BOTCUtils.get_player_from_id(ctx.author.id)
        await player.role.ego_self.register_execute(player, executed)
