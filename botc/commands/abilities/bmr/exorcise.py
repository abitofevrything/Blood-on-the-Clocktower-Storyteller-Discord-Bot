...
from botc import check_if_is_player, check_if_is_night, check_if_dm, RoleCannotUseCommand, \
    check_if_player_really_alive, check_if_can_exorcise, PlayerParser, AbilityForbidden, \
...
class Exorcise(commands.Cog, name = documentation["misc"]["abilities_cog"]):
    ...
    @commands.command(
        ...
        name = "exorcise",
        hidden = False,
        brief = documentation["doc"]["exorcise"]["brief"],
        help = documentation["doc"]["exorcise"]["help"],
        description = documentation["doc"]["exorcise"]["description"]
    )
    ...
    @commands.check(check_if_can_exorcise)  # Correct character -> RoleCannotUseCommand
    async def protect(self, ctx, *, exorcised: PlayerParser()):
        """Protect command
        usage: exorcise <player> and <player> and...
        characters: exorcist
        """

        player = BOTCUtils.get_player_from_id(ctx.author.id)
        await player.role.ego_self.register_exorcise(player, exorcised)
